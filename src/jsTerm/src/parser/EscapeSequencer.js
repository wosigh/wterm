...
TERM.EscapeSequencer = function (viewer){
	...
	this.init = function() {
		...
		this.commandsESC['8'] = this.dec8; //this.cursorRestore
		this.commandsESC['c'] = this.reset;
		this.commandsESC['A'] = this.charsetUK;
		this.commandsESC['B'] = this.charsetUS;
		this.commandsESC['0'] = this.charsetSpecial;
		this.commandsESC['1'] = this.charsetAlt;
		this.commandsESC['2'] = this.charsetAltSpecial;
		this.commandsESC['7'] = this.cursorSave;

		this.commandsESC['D'] = this.index;
		this.commandsESC['M'] = this.reverseIndex;
		this.commandsESC['E'] = this.nextLine;
		this.commandsESC['H'] = this.setTabStop;

		this.commandsCSI['h'] = this.modeSet;
		this.commandsCSI['l'] = this.modeReset;

		this.commandsCSI['c'] = this.deviceAttributes;
		this.commandsCSI['m'] = this.setGraphicsMode;
		this.commandsCSI['J'] = this.eraseDisplay;
		this.commandsCSI['K'] = this.eraseLine;	
		this.commandsCSI['r'] = this.scrollScreen;
		this.commandsCSI['H'] = this.cursorPosition;
		this.commandsCSI['f'] = this.cursorPosition;
		this.commandsCSI['A'] = this.cursorUp;	
		this.commandsCSI['B'] = this.cursorDown;	
		this.commandsCSI['C'] = this.cursorForward;	
		this.commandsCSI['D'] = this.cursorBackward;
		this.commandsCSI['d'] = this.cursorFromTop;
		this.commandsCSI['G'] = this.cursorFromLeft;
		this.commandsCSI['P'] = this.deleteChar;
		this.commandsCSI['X'] = this.eraseChar;
		this.commandsCSI['L'] = this.insertLine;
		this.commandsCSI['n'] = this.deviceStatusReport;
		this.commandsCSI['p'] = this.softreset;
		this.commandsCSI['M'] = this.deleteLines;
		this.commandsCSI['g'] = this.clearTabStops;

	};

	this.executeESC = function(ic, fc) {
		if (this.commandsESC[fc] != undefined)
			this.commandsESC[fc](ic);
		else
			enyo.error('ESC', ic, fc)
	};
	
	this.executeCSI = function(cmd, paramsRaw) {
		var params = paramsRaw.split(';')
		if (this.commandsCSI[cmd] != undefined)
			this.commandsCSI[cmd](params);
		else
			enyo.error('CSI', cmd, params)
	};
